...
public:
    ...
    uint16_t get_period() const { return period; }

    void set_hi(uint8_t hi) { period = (period & 0x00FF) | ((hi & 0x7) << 8); }//= (period & 0xF) | ((hi & 0x7) << 8); }
    void set_lo(uint8_t lo) { period = (period & 0x7F00) | lo; }
private:
    uint16_t period = 0;
    uint16_t counter = 0;
};
...
public:
    Envelope() : div(15) {};
    void clock();
    uint8_t get_volume() const { return constant ? param : decay; }

    void set_param(uint8_t p) { param = p; }
    void set_loop(bool l) { constant = l; }
    void set_constant_vol(bool c) { constant = c; }
